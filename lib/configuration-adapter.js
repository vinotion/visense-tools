 ...
 *        be selected if an array of multiple data values is provided.
 *
...
function extractParameterValue(response) {
  function iterate(input) {
    if (typeof input === 'undefined') {
      throw new Error('Unexpected undefined input');
    }
 ...
 */
function ConfigurationAdapter(socketAddress, useSsl, sessionToken, timeout) {
  ...
  const _defaultOptions = {
    url: (useSsl ? 'https://' : 'http://') + socketAddress.ip + ':' + socketAddress.port + '/api/v2/config',
    headers: {
  ...
  function get(name, type) {
    return got.get(_defaultOptions.url, {
      searchParams: { Command: 'Get', Select: name, Filter: type, Format: 'Json-Value-v1' },
      headers: _defaultOptions.headers,
      responseType: 'json',
      https: { rejectUnauthorized: false },
      timeout: { request: _defaultOptions.timeout }
  ...
  function set(name, type, value) {
    return got.get(_defaultOptions.url, {
      searchParams: { Command: 'Set', [name]: value, Filter: type, Format: 'Json-Value-v1' },
      headers: _defaultOptions.headers,
      responseType: 'json',
      https: { rejectUnauthorized: false },
      timeout: { request: _defaultOptions.timeout }
