 ...
 */
function ViSenseSystem(socketAddress, useSsl, sessionToken, id, timeout) {
  // Assign user-specified ID.
  if (checkUtils.isDefinedNonString(id)) {
    throw new Error('Parameter \'id: ""\' has invalid type');
  }
  const _id = (typeof id === 'undefined') ? socketAddress.ip : id;

  // Other function parameters are checked downstream.
  const _configAdapter = ConfigurationAdapter(socketAddress, useSsl, sessionToken, timeout);
